...
function saveUser(req, res){
	...
	if(params.name &&  params.password &&
	   params.nick && params.email && params.politics){
...
async function getCountFollow(user_id){
	...
	var uservs = await Like.count({"user":user_id}).exec((err, count) => {
		if(err) return handleError(err);
		return count;
	});

	var likeds = await Like.count({"liked":user_id}).exec((err, count) => {
		if(err) return handleError(err);
		return count;
	});

	var publicationvs = await Like.count({"publication":user_id}).exec((err, count) => {
		if(err) return handleError(err);
		return count;
	});
